# 한화시스템 Beyond SW 21기 백엔드 프로젝트 - READWE

---
# 😄 팀 소개
| [김수진](https://github.com/whatimeisitnow) | [김세현](https://github.com/sekong11) | [이용우](https://github.com/L-dragon-woo) | [정재우](https://github.com/NoSa1e) | 
| :-: | :-: | :-: | :-: |
| <img src="https://github.com/user-attachments/assets/45e223f8-6216-4ea3-825b-45624d38bab9" width="200" /> | <img src="https://github.com/user-attachments/assets/a449cd98-e530-42fe-91ca-07c1ce519ddf" width="200" /> |<img src="https://github.com/user-attachments/assets/914bc401-29dd-4b04-98ac-107cbe153b6d" width="200" /> |<img src="https://github.com/user-attachments/assets/447ad558-3566-4a48-b20e-8cc100176de2" width="200" /> | 

---
# 🛠️ 기술스택
---
## 백엔드
<img src="https://img.shields.io/badge/spring-6DB33F?style=for-the-badge&logo=spring&logoColor=white"> <img src="https://img.shields.io/badge/springboot-6DB33F?style=for-the-badge&logo=springboot&logoColor=white"> <img src="https://img.shields.io/badge/springsecurity-6DB33F?style=for-the-badge&logo=springsecurity&logoColor=white"> 
<img src="https://img.shields.io/badge/java-007396?style=for-the-badge&logo=java&logoColor=white"> <br>
<img src="https://img.shields.io/badge/mariaDB-003545?style=for-the-badge&logo=mariaDB&logoColor=white"> <img src="https://img.shields.io/badge/redis-FF4438?style=for-the-badge&logo=redis&logoColor=white"> <img src="https://img.shields.io/badge/postman-FF6C37?style=for-the-badge&logo=postman&logoColor=white"> <img src="https://img.shields.io/badge/swagger-85EA2D?style=for-the-badge&logo=swagger&logoColor=white"> 
<img src="https://img.shields.io/badge/gradle-02303A?style=for-the-badge&logo=gradle&logoColor=white">
---
# ℹ️ 프로젝트 소개

## 1. 프로젝트 개요

ReadWe는 독서를 좋아하는 사람들이 더욱 편리하게 소통하고, 자신의 독후감을 체계적으로 기록하고 업로드 할 수 있도록 개발된 독서플랫폼 입니다.

## 2. 개발 배경

독서를 좋아하는 사람은 자신의 독서 기록을 남기거나 다른 사람의 독후감을 참고하며 독서 경험을 확장하려는 경향이 있습니다. 하지만 이러한 활동은 대부분 개인 블록, 메모, sns등 서로 다른 도구에 분산되어 있습니다.

즉, 책에 대한 생각을 다른사람과 공유하고 싶어도, 이를 다루는 플랫폼은 제한적이며, 기존 서비스들은 독서 기록이나 리뷰에만 초점이 맞춰져 있어 독서모임을 직접 계획하고 운영하는 기능은 부족합니다.

따라서 ReadWe는 독서 기록 관리 → 독후감 공유 → 독서모임 계획 및 운영까지 하나의 흐름으로 연결해, 독서 활동 전반을 통합적으로 지원하는 플랫폼을 목표로 합니다.

## 3. 프로젝트 목적

- 사용자가 자신의 독서를 기록하고 정리할 수 있는 환경 제공
- 다른 사람의 독후감을 쉽게 조회하고 공유할 수 있는 커뮤니티 기반 마련
- 단순 리뷰 공유를 넘어, 사용자들이 직접 독서모임을 계획하고 운영할 수 있는 기능 제공
- 독서활동 전반(기록-공유-모임)을 하나의 플랫폼에서 처리할 수 있는 통합적 사용자 경험 구축

## 4. 주요 제공 가치

- 기록의 지속성
    
    사용자는 독후감과 리뷰를 플랫폼 내에 저장하여 개인의 독서 역사를 체계적으로 남길 수 있습니다.
    
- 공유와 소통의 확대
    
    다른 사용자의 독후감을 읽고 댓글을 통해 의견을 주고받으며 자연스러운 독서 커뮤니티 활동이 가능합니다.
    
- 독서모임 기획 기능 제공
    
    기존 독서플랫폼에서 다루기 어려웠던 ‘독서모임 생성’기느응ㄹ 제공하여 단순 기록을 넘어 함께 읽기 활동 까지 확장된 경험을 제공합니다.

# 📈 시장 조사
<details>
<summary>독서모임 및 독서 인기</summary>
<div markdown="1">
<img width="1939" height="1266" alt="image" src="https://github.com/user-attachments/assets/e9c58178-45b5-404b-b19f-7492f292ca45" />
<img width="1653" height="1272" alt="image" src="https://github.com/user-attachments/assets/4069786f-7cac-4f6d-b708-17ef912aae94" />
<img width="1705" height="1311" alt="image" src="https://github.com/user-attachments/assets/8da3f999-992d-4c6e-8fe3-6bbf8851266e" />


</div>
</details>
<details>
<summary>독서모임의 필요성</summary>
<div markdown="1">
  
- **독서 습관 형성 및 완독 동기 부여**: 혼자 읽기 어려운 두꺼운 책이나 전문 서적을 정해진 분량만큼 읽고 토론하며 완독을 이끌어냅니다.
- **사고 확장 및 통찰력 향상**: 다양한 배경의 사람들과 의견을 주고받으며, 자신만의 익숙한 시각을 넘어 새로운 관점을 배우고 깊이 있는 이해를 얻습니다.
- **소통 능력 및 표현력 증진**: 자신의 생각을 정리하고 논리적으로 표현하는 과정에서 어휘력, 감수성, 창의적 의사소통 능력이 향상됩니다.
- **긍정적이고 건강한 관계 형성**: 같은 가치를 공유하는 사람들과의 만남을 통해 긍정적인 자극을 받고, 서로를 이해하며 신뢰 기반의 관계를 형성할 수 있습니다.
- **자기 효능감 및 자존감 향상**: 독서와 성장의 과정을 함께하며 스스로를 발전시킨다는 성취감을 느끼고, 스트레스 해소 및 자아실현의 기회가 됩니다.
- **조직 문화 개선 및 협업 강화**: 직장 내 독서모임은 구성원 간 신뢰를 쌓고 소통을 활성화하여 협업을 강화하는 효과적인 도구가 될 수 있습니다
</div>
</details>

# 🔎 유사 프로그램 분석
<details>
<summary>독서모임 플랫폼</summary>
<div markdown="1">

## 온라인
<details>
<summary> 그믐: 자생적으로 성장한 온라인 독서 모임 플랫폼으로, 국내외 회원이 참여하며 글쓰기, 댓글, 문장 수집 등 자유로운 활동이 가능 </summary>
<div markdown="1">

<img width="2849" height="1603" alt="image" src="https://github.com/user-attachments/assets/2fce282a-6f6a-4d44-a0a0-0ff603f14a2d" />

</div>
</details>
<details>
<summary> 플라이북: 독서 소셜 네트워크 플랫폼으로, 독서 기록 및 관련 활동을 지원 </summary>
<div markdown="1">

<img width="2864" height="1607" alt="image" src="https://github.com/user-attachments/assets/7aee60ee-bbfd-423a-9e3c-bfc25ce8e0fd" />

</div>
</details>
<details>
<summary> 북모리: 독서 기록 앱으로 커뮤니티 기능이 포함 </summary>
<div markdown="1">

<img width="2824" height="1482" alt="image" src="https://github.com/user-attachments/assets/d51d284b-ce1f-4273-8950-5964bdf03467" />

</div>
</details>

## 오프라인
<details>
<summary>트레바리: 유료 모임으로, 특정 책을 읽고 독후감을 공유하며 토론하는 모임. 1년에 4번, 1달에 1번씩 모임을 가지며, 모임 후 뒤풀이나 친목 도모 활동도 진행</summary>
<div markdown="1">

<img width="2604" height="1459" alt="image" src="https://github.com/user-attachments/assets/2ef2c5f9-23df-4e6c-8e38-b6c53d86594b" />


</div>
</details>
<details>
<summary>북덕방: 앱 기반의 독서 모임 플랫폼</summary>
<div markdown="1">

<img width="2787" height="1362" alt="image" src="https://github.com/user-attachments/assets/0c500242-8648-405c-a8fb-c5c288205193" />



</div>
</details>

</div>
</details>

# 💡 주요 기능
| 구분 | 기능명 | 설명 |
| :-- | :-- | :-- |
| 사용자 | 회원관리 | 회원가입, 로그인, 로그아웃, 회원정보 수정 |
| 사용자 | 책 조회/좋아요 | 검색, 좋아요, 랭킹, 선호카테고리 |
| 사용자 | 독후감/댓글 | 독후감 CRUD, 댓글, 좋아요 |
| 사용자 | 독서 모임 | 모임 생성, 가입, 리뷰CRUD·댓글·좋아요 |
| 관리자 | 책 관리 | 책 추가, 삭제 |
<details>
<summary>기능 상세</summary>
<div markdown="1">

## 1. **회원(Member) 기능**

- 회원가입 / 로그인 / 로그아웃
- 닉네임 변경
- 비밀번호 변경

---

## 2. **책(Book) 기능**

- 책 리스트 조회
    - 제목, 카테고리, 작가명 기준 조회
- 책 좋아요
    - 좋아요 등록 / 취소
- 책 랭킹 조회
- 사용자 선호 카테고리 관리
    - 카테고리 추가 / 삭제 / 조회

---

## 3. **독후감(Book Report) 기능**

- 독후감
    - 등록 / 조회 / 수정 / 삭제
- 독후감 댓글 기능
    - 댓글 등록 / 조회 / 수정 / 삭제
- 독후감 대댓글 기능
    - 대댓글 등록 / 조회 / 수정 / 삭제
- 독후감 좋아요 등록 / 취소

---

## 4. **독서모임(Book Club) 기능**

- 독서모임 생성 / 조회
- 독서모임 가입 관리
    - 사용자 가입 / 가입 취소
- 모임 리뷰 기능
    - 리뷰 등록 / 수정/ 삭제
    - 리뷰 좋아요순, 최신순 조회
    - 리뷰 댓글 등록 / 조회 / 수정 / 삭제
    - 리뷰 대댓글 등록 / 조회 / 수정 / 삭제
    - 리뷰 좋아요 등록 / 취소

</div>
</details>

# 기대 효과

# ERD 및 주요 도메인
## 요구사항 명세서
## DDD
<details>
<summary>miro</summary>
<div markdown="1">

<img width="2112" height="1460" alt="image" src="https://github.com/user-attachments/assets/80f4b5df-07c9-402f-b240-1ad0f890c5bc" />

</div>
</details>

## 논리 데이터 베이스 모델
<details>
<summary>ERD</summary>
<div markdown="1">

<img width="1901" height="872" alt="image" src="https://github.com/user-attachments/assets/892d444f-51c2-403e-8e7d-ecdd82386cb2" />
ERD 링크
[https://www.erdcloud.com/d/pB3faMZEj7tvLtfH3](https://www.erdcloud.com/d/STZY9fcram2cvZvb5)

</div>
</details>

## API 구조

## POST MAN

## Swagger

<details>
<summary> swagger</summary>
<div markdown="1">
<details>
<summary> User-Service</summary>
<div markdown="1">
<img width="757" height="768" alt="스크린샷 2025-12-11 오후 6 21 05" src="https://github.com/user-attachments/assets/d0c77961-b5dd-42c1-a5f5-25158f7e2476" />

</div>
</details>
<details>
<summary> ReadingClub-Service</summary>
<div markdown="1">
<img width="748" height="906" alt="스크린샷 2025-12-11 오후 6 21 32" src="https://github.com/user-attachments/assets/77eb7b17-cf26-49bc-b2ba-e7d3baf3086a" />
<img width="734" height="509" alt="스크린샷 2025-12-11 오후 6 21 43" src="https://github.com/user-attachments/assets/4573dc61-00d9-4014-91c1-8e62b197c937" />
    
</div>
</details>
<details>
<summary> Book-Service</summary>
<div markdown="1">
<img width="749" height="917" alt="스크린샷 2025-12-11 오후 6 22 11" src="https://github.com/user-attachments/assets/b01a9a42-7250-4266-933b-0d9173bc70b0" />
<img width="737" height="509" alt="스크린샷 2025-12-11 오후 6 22 23" src="https://github.com/user-attachments/assets/42b86c96-8ad4-4941-8362-fe404c67ec74" />


</div>
</details>

</div>
</details>

## 테스트 케이스 상세
<details>
<summary> 세부정보</summary>
<div markdown="1">

### 회원 관리 기능
<details>
<summary> 토글 이름 </summary>
<div markdown="1">
토글 내용, 이미지
</div>
</details>

### 관리자 기능
<details>
<summary> 토글 이름 </summary>
<div markdown="1">
토글 내용, 이미지
</div>
</details>

### 책 기능
<details>
<summary> 토글 이름 </summary>
<div markdown="1">
토글 내용, 이미지
</div>
</details>

### 독후감 기능
<details>
<summary> 토글 이름 </summary>
<div markdown="1">
토글 내용, 이미지
</div>
</details>

### 독서 모임 기능
<details>
<summary> 독서 모임 생성 </summary>

![Image](https://github.com/user-attachments/assets/302d3868-9ef2-4010-b511-0830cbbbeef8)

</details>
<details>
<summary> 독서 모임 수정 </summary>

![Image](https://github.com/user-attachments/assets/7de0d8e6-064f-46eb-bbf6-f51a2cb976ba)

</details>
<details>
<summary> 독서 모임 신청 </summary>

![Image](https://github.com/user-attachments/assets/b0d9d0aa-3845-42cf-8c65-51fbf8929209)

</details>
<details>
<summary> 독서 모임 신청 승인 </summary>

![Image](https://github.com/user-attachments/assets/cdae9f38-fa0f-4bf7-8157-1f4dcf133309)

</details>
<details>
<summary> 모임이 받은 신청 조회 </summary>

![Image](https://github.com/user-attachments/assets/63e5c34d-84d3-4251-aaaa-b15de3c493da)

</details>
<details>
<summary> 모임 멤버 조회 </summary>

![Image](https://github.com/user-attachments/assets/a47a0344-8cbc-41b4-ad9f-af9722230d5e)

</details>
<details>
<summary> 내 모임 조회 </summary>

![Image](https://github.com/user-attachments/assets/70be826a-0d8f-47f1-8620-181cd169ae73)

</details>
<details>
<summary> 내 가입 신청 조회 </summary>

![Image](https://github.com/user-attachments/assets/f93f76ed-61e9-444c-8042-b291a85dffa4)

</details>
<details>
<summary> 모임 해산 </summary>

![Image](https://github.com/user-attachments/assets/37242566-7c1c-4b91-af84-f739be24bd2d)

</details>




</div>
</details>

## MSA 아키텍처 구조도

<details>
<summary> MSA 아키텍처 구조도</summary>
<div markdown="1">
<img width="1852" height="881" alt="image" src="https://github.com/user-attachments/assets/e6791924-0358-418b-b608-c07393b07d98" />

</div>
</details>

# 팀원 회고
<details>
<summary> 김수진 </summary>
<div markdown="1">
회고내용
</div>
</details>
<details>
<summary> 김세현 </summary>
<div markdown="1">
회고내용
</div>
</details>
<details>
<summary>이용우</summary>
<div markdown="1">

이번 프로젝트 아이디어는 제가 예전에 참여했던 독서모임 경험에서 출발했습니다.
그때 여러 사람들과 책을 함께 읽고 이야기를 나누며 큰 즐거움을 느꼈고,
“이런 모임이 더 많아지고 많은 사람들이 책을 통해 연결될 수 있으면 좋겠다”는 생각이 자연스럽게 들었습니다.
팀원분들도 이 아이디어에 긍정적으로 공감해주셔서 프로젝트로 선정할 수 있었습니다.

진행 과정에서 아쉽게도 팀원 한 분이 건강 문제로 함께하지 못하게 되었지만,
남은 팀원들이 서로 힘을 모아 빠르게 일치단결했고
각자 맡은 역할을 책임감 있게 수행해주어 프로젝트를 끝까지 이어갈 수 있었습니다.

특히 밤을 새워가며 개발하고 문서를 정리하며
서로 도와주는 모습을 보면서 진심으로 감동했고,
그 모습이 저에게도 더 열심히 하고 싶다는 큰 동기부여가 되었습니다.

함께 고생해준 팀원 모두에게 감사드리고,
이 경험을 통해 협업의 소중함과 공동의 목표를 향해 나아가는 즐거움을 다시 한번 느낄 수 있었습니다.


</div>
</details>
<details>
<summary> 정재우 </summary>
<div markdown="1">
회고내용
</div>
</details>




